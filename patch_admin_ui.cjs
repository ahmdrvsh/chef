...
const uiOld = `      {/* آمار کلی */}
      ...
      <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
         <h2 className="text-xl font-bold mb-4 text-gray-800">پربازدیدترین غذاها</h2>
           ...
           <table className="w-full text-right">
             <thead>
               <tr className="border-b border-gray-100 text-sm text-gray-500">
                 ...
                 <th className="py-3 font-medium">تعداد بازدید</th>
                   ...
                   replyingTo === c.id ? (
                     <div className="flex gap-2">
                       <input 
                         ...
                         className="flex-1 text-sm border border-gray-300 rounded-lg px-3 py-1.5 focus:ring-1 focus:ring-orange-500"
                       />
                       <button onClick={() => submitReply(c.id)} className="bg-orange-500 text-white px-4 py-1.5 rounded-lg text-sm font-bold">ثبت</button>
                       <button onClick={() => setReplyingTo(null)} className="bg-gray-200 text-gray-700 px-4 py-1.5 rounded-lg text-sm font-bold">لغو</button>
                     </div>
                   ) : (
                     <button onClick={() => setReplyingTo(c.id)} className="text-orange-600 hover:underline text-xs font-bold">پاسخ به این نظر</button>
                   )
                 )}
               </div>
             ))}
